...
- 视频一律 `muted`（现在的片子也已经没有音轨），避免任何自动出声的意外；
...
实测（iPhone 视口 390×844、DPR 3、4G 节流）：首屏从 **27.34MB** 降到 **0.65MB**，滚完整页从 40MB 上下降到 **3.29MB**。封面视频改成点击播放之后，首屏和整页都不再包含任何视频；用户真去点播放时，单段再花 1.1–3.3MB。
